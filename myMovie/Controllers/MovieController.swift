...
//    }
    ...
    func deleteMovieFromDbWith(indexSet: IndexSet){
//        let index = movieList.firstIndex {
//            $0.id == movie.id
//        }
//        if let index{
//            dbService.delete(item: movieList[index])
//        }
    ...
    func getAll(){
        Task {
            movieList = await dbService.getAll()
        }
    }
    ...
    func addMovieToDbWith(title: String, year: String, rating: Double, resume: String, urlString: String){
        let movie = Movie(title: title, year: year, rating: rating, resume: resume, urlString: urlString)
        let newId = dbService.add(item: movie)
