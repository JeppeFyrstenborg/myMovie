...
struct NavigationButtonsView: View {
    ...
    var body: some View {
        let currIndex = movieViewModel.IndexOfThis(movie: movie)
        Spacer()
            .frame(height: 20)
        VStack{
            HStack{
                let prevMovie = movieViewModel.CheckForPrevMovieWith(currIndex: currIndex)
                if prevMovie != nil{
                    PrevBtnView(clickable: true, movie: prevMovie!)
                }
                Spacer()
                let nxtMovie = movieViewModel.CheckForNxtMovieWith(currIndex: currIndex)
                if nxtMovie != nil{
